...
import model.ViewProductsSellerDaoModel;
...
public class ViewProductsSellerController {
    ...
    @FXML
    private TableColumn<Product, Number> itemPriceCol;
    @FXML
    private TableColumn<Product, Number> itemQuantityCol;
    @FXML
    private TableColumn<Product, String> itemSKUCol;

    private ViewProductsSellerDaoModel productDAO = new ViewProductsSellerDaoModel();
    ...
    private void loadProducts() {
        int sellerId = getCurrentSellerId();  // Assume you have a method to get the current seller's ID
        ObservableList<Product> products = FXCollections.observableArrayList(productDAO.getProductsBySeller(sellerId));
        tableView.setItems(products);
    }

    private int getCurrentSellerId() {
        // Mocked seller ID for demonstration. Replace with dynamic fetching logic.
        return 1;  // Assume seller ID 1 for demonstration
    }
}
...
//                   rs.getInt("user_id")
    ...
    private void loadScene(String fxmlPath) {
        try {
            ...
            Stage stage = (Stage) tableView.getScene().getWindow(); // Assuming 'tableView' is part of the current scene
