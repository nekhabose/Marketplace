        ...
        try (Connection conn = new DBConnect().connect();
            ...
            while (rs.next()) {
                orders.add(new Order(
                    rs.getInt("order_id"),
                    rs.getInt("user_id"), // Adding user_id fetched from the session
...
//            if (!updateProductQuantity(conn, productId, quantity)) {
//                conn.rollback();
//                return false;
//            }
...
//    private boolean updateProductQuantity(Connection conn, int productId, int quantityChange) throws SQLException {
